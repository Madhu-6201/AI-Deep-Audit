...
st.write("This dashboard updates automatically after single transaction predictions.")

if len(st.session_state.history) > 0:
    dashboard_df = pd.DataFrame(st.session_state.history)
else:
    dashboard_df = pd.DataFrame(columns=["Time", "Risk_Score", "Status"])

total_predictions = len(dashboard_df)
high_risk_count = 0
safe_count = 0
avg_risk_score = 0.0
last_status = "No Prediction Yet"

if total_predictions > 0:
    high_risk_count = dashboard_df["Status"].str.contains("High Risk|Fraud|Flagged", case=False, na=False).sum()
    safe_count = dashboard_df["Status"].str.contains("Safe|Verified|Low Risk", case=False, na=False).sum()
    avg_risk_score = dashboard_df["Risk_Score"].mean()
    last_status = dashboard_df.iloc[-1]["Status"]

kpi1, kpi2, kpi3, kpi4, kpi5 = st.columns(5)
kpi1.metric("Total Predictions", int(total_predictions))
kpi2.metric("High Risk Cases", int(high_risk_count))
kpi3.metric("Safe Cases", int(safe_count))
kpi4.metric("Average Risk", f"{avg_risk_score:.2f}%")
kpi5.metric("Last Status", last_status)
...
with chart_col2:
    ...
    if total_predictions > 0:
        trend_df = dashboard_df[["Time", "Risk_Score"]].copy().set_index("Time")
        st.line_chart(trend_df)
    else:
        st.info("Risk trend will appear after predictions.")
        ...
        if missing_cols:
            ...
            if st.button("Run Batch Fraud Detection", use_container_width=True):
                ...
                if result_df is None:
                    ...
                    st.write("This dashboard is generated from the uploaded dataset prediction results.")
                    ...
                    u1.metric("Total Transactions", int(total_rows))
                    u2.metric("High Risk / Fraud", int(fraud_count))
                    u3.metric("Safe Transactions", int(safe_count))
                    u4.metric("Fraud Percentage", f"{fraud_percentage:.2f}%")

                    u5, u6, u7 = st.columns(3)
                    u5.metric("Average Risk Score", f"{avg_risk:.2f}%")
                    u6.metric("Maximum Risk Score", f"{max_risk:.2f}%")
                    u7.metric("Minimum Risk Score", f"{min_risk:.2f}%")
                    ...
                    with c_left:
                        ...
                        st.bar_chart(upload_summary_df.set_index("Prediction"))
